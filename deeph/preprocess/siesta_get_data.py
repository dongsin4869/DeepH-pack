...
def siesta_parse(input_path, output_path):
    ...
    with open(
        "{}/{}.STRUCT_OUT".format(input_path, system_name), "r"
    ) as struct:  # structure info from standard output
    ...
    atom_coord_cart = np.genfromtxt(
        "{}/{}.XV".format(input_path, system_name), skip_header=4
    )
    atom_coord_cart = atom_coord_cart[:, 2:5] * 0.529177249
    np.savetxt(
        "{}/site_positions.dat".format(output_path), np.transpose(atom_coord_cart)
    )

    orb_indx = np.genfromtxt(
        "{}/{}.ORB_INDX".format(input_path, system_name), skip_header=3, skip_footer=17
    )
    ...
    with open("{}/orbital_types.dat".format(output_path), "w") as orb_type_f:
        ...
        while True:  # Loop over atoms in unitcell
            if atom_current != orb_indx[i, 1]:
                ...
                atom_orb_cnt = np.array(
                    [0, 0, 0, 0]
                )  # number of s, p, d, f orbitals in specific atom
            l = int(orb_indx[i, 6])
            atom_orb_cnt[l] += 1
            i += 1
            if i > len(orb_indx) - 1:
    ...
    atom_current = 1
    orb_atom_current = np.empty(
        (0)
    )  # stores orbitals' id in siesta, n, l, m and z, will be reshaped into orb*5
    t = 0
    for i in range(len(orb_indx)):
        ...
        if i != len(orb_indx) - 1:
            if orb_indx[i + 1, 1] != atom_current:
                orb_atom_current = np.reshape(
                    orb_atom_current, ((int(len(orb_atom_current) / 5), 5))
                )
